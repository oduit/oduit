...
def get_config_path(env_name: str) -> str:
    """Returns full path to environment-specific config."""
    config_path, _ = _default_config_loader._detect_config_format(env_name)
    return config_path
...
def load_config(env_name: str) -> dict[str, Any]:
    """Load config from ~/.config/oduit/<env>.(yaml|toml|conf) or from a
    direct file path"""
    return _default_config_loader.load_config(env_name)


def has_local_config() -> bool:
    """Check if a local .oduit.toml file exists in current directory."""
    return _default_config_loader.has_local_config()


def load_local_config() -> dict[str, Any]:
    """Load config from .oduit.toml in current directory."""
    return _default_config_loader.load_local_config()


def get_available_environments() -> list[str]:
    """Return a list of available environment names based on config files."""
    return _default_config_loader.get_available_environments()


def load_demo_config() -> dict[str, Any]:
    """Load a demo configuration for testing without a real Odoo server"""
    return _default_config_loader.load_demo_config()
